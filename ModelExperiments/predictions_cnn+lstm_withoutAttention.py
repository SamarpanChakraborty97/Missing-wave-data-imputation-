        ...
        with torch.no_grad():
            ...
            if (num == 0):    
                for i in range(math.floor(len(test)/missing_len)):
                    ...
                    im = plt.contourf(time, frequencies, power, cmap = cmap, vmin=p_l, vmax=p_h, levels=levs, extend='both')
                    plt.axis('off')
                    plt.savefig(f'Sample3.jpeg', bbox_inches='tight',pad_inches = 0)
                    plt.close()

                    image = Image.open(f'Sample3.jpeg')
...
PATH = f"cnn_lstm_withoutAttention_twoFreqs.pt"
...
np.savetxt("twoFreqs_cnn2d+lstm_withoutAttention_pred_results.out", Predictions)
