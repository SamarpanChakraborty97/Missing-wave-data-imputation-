...
for i in range(1,6):
    data_pre = pd.read_csv(f"EleFit_pre_{i}.csv", header=None)
    data_post = pd.read_csv(f"EleFit_post_{i}.csv", header=None)
    data_whole = pd.read_csv(f"EleFit_whole_{i}.csv", header=None)
    ...
    for j in range(len(data)):
        ...
        X_train_pre_CNN_tensor = torch.zeros(len(Train_X_pre), 3 , 77, 77)
        ...
        X_val_pre_CNN_tensor = torch.zeros(len(Val_X_pre), 3 , 77, 77)
    ...
    np.savetxt(f"Preds_cnn+lstm_{i}.out", Preds_rescaled)
