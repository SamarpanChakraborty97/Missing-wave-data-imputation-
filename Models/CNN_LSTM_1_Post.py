    ...
    def evaluate(self, x, test, training_len, output_len, missing_len):
        with torch.no_grad():
            ...
            for i in range(missing_len):
                ...
                plt.xlim([20, 150])
                plt.ylim([0.005, 0.04])
                plt.savefig(f'SamplePred.jpeg', bbox_inches='tight',pad_inches = 0)
                plt.close()

                image = Image.open(f'SamplePred.jpeg')
...
for i in range(1,2):
    ...
    for j in range(len(pre_data_scaled)):
        ...
        for k in range(L1):
            X_pre[k,:] = pre_data_scaled[j,k:k+input_len]
            Y_pre[k,:] = pre_data_scaled[j,k+input_len:k+input_len+output_len]
        ...
        X_train_pre_lstm_tensor = torch.Tensor(Train_X_pre.copy())
        Y_train_pre_tensor = torch.Tensor(Train_Y_pre.copy())
    
        X_val_pre_lstm_tensor = torch.Tensor(Val_X_pre.copy())
        Y_val_pre_tensor = torch.Tensor(Val_Y_pre.copy())
        
        X_train_pre_CNN_tensor = torch.zeros(len(Train_X_pre), 3 , 53, 54)
        for m in range(len(X_train_pre_CNN_tensor)):            
            ...
            X_train_pre_CNN_tensor[m] = tensor

        X_val_pre_CNN_tensor = torch.zeros(len(Val_X_pre), 3 , 53, 54)
        for m in range(len(X_val_pre_CNN_tensor)) :  
            ...
            plt.xlim([20, 150])
            plt.ylim([0.005, 0.04])
            plt.savefig(f'SampleVal.jpeg', bbox_inches='tight',pad_inches = 0)
            plt.close()
        
            image = Image.open(f'SampleVal.jpeg')
            tensor = transform(image)
            X_val_pre_CNN_tensor[m] = tensor
    
        mode = j
    
        torch.manual_seed(2)
    
        train_pre_eta = CNN_LSTM_Dataset(X_train_pre_CNN_tensor, X_train_pre_lstm_tensor, Y_train_pre_tensor)
        val_pre_eta = CNN_LSTM_Dataset(X_val_pre_CNN_tensor, X_val_pre_lstm_tensor, Y_val_pre_tensor)
