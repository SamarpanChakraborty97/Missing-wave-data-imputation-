...
CNN_LSTM_Fit = zeros(Num_Windows,3);    t_CNN_LSTM_Fit = zeros(Num_Windows,2);    Max_CNN_LSTM_Fit = zeros(Num_Windows,1);
CNN_LSTM_Slow = zeros(Num_Windows,3);   t_CNN_LSTM_Slow = zeros(Num_Windows,2);   Max_CNN_LSTM_Slow = zeros(Num_Windows,1);
...
BNN_LSTM_Slow = zeros(Num_Windows,3);   t_BNN_LSTM_Slow = zeros(Num_Windows,2);   Max_BNN_LSTM_Slow = zeros(Num_Windows,1);
BNN_LSTM_Fit = zeros(Num_Windows,3);    t_BNN_LSTM_Fit = zeros(Num_Windows,2);    Max_BNN_LSTM_Fit = zeros(Num_Windows,1);

w_start = initial_window_start : 18960 : (18960*Num_Max_Windows)+initial_window_start;
rand_sample = 75;
for i=rand_sample:rand_sample
    ...
    hz=ncread(src,'xyzZDisplacement',w_start(j),window_size_tot).';
    ...
    BNN=[];
    for p=1:10
        name = sprintf('Preds_bnn+lstm_%i_%i.out',i,p);
        BNN(i,:) = dlmread(name);
    end    
    bnn_lstm_fit = mean(BNN,1);
    %%%%%%%%%%%%%%%%%%%
    
    %%%%%% BNN+LSTM_AMPLITUDES %%%%%%%
    BNN=[];
    for p=1:5
        name = sprintf('Preds_pre_bnn+lstm_%i_%i.out',i,p);
        b = dlmread(name);
        BNN(i,:)=SF2fulltime(t_miss_plot,t_miss_plot,b,freqs,N_fmodes);
    end
    hz_slowflow_full_bnn_lstm=mean(BNN,1);
    %%%%%%%%%%%%%%%%%%%
...
%     hz_slowflow_full_baseline=SF2fulltime(t_miss_plot,t_miss_plot,baseline,freqs,N_fmodes);
    ...
    [bnn_lstm_fit_wave_height ,bnn_lstm_fit_forecast_time,~, ~]=my_wave_height_filter(bnn_lstm_fit, N_pts_per_wave);
    [bnn_lstm_amp_wave_height ,bnn_lstm_amp_forecast_time,~, ~]=my_wave_height_filter(hz_slowflow_full_bnn_lstm, N_pts_per_wave);
    ...
    [h_cmp_bnn_fit_lstm,t_cmp_bnn_fit_lstm] = my_wave_height_compare(true_height,true_time,bnn_lstm_fit_wave_height,bnn_lstm_fit_forecast_time);
    [h_cmp_bnn_amp_lstm,t_cmp_bnn_amp_lstm] = my_wave_height_compare(true_height,true_time,bnn_lstm_amp_wave_height,bnn_lstm_amp_forecast_time);
    ...
    h_cmp_bnn_lstm_fit_1min = h_cmp_bnn_fit_lstm(1:idx_1min);    % BNN+LSTM 1 minute peaks
    t_cmp_bnn_lstm_fit_1min = t_cmp_bnn_fit_lstm(1:idx_1min);    % BNN+LSTM 1 minute peak times
    
    h_cmp_bnn_lstm_amp_1min = h_cmp_bnn_amp_lstm(1:idx_1min);    % BNN+LSTM SLOW 1 minute peaks
    t_cmp_bnn_lstm_amp_1min = t_cmp_bnn_amp_lstm(1:idx_1min);    % BNN+LSTM SLOW 1 minute peak times
    ...
    cnn_lstm_fit_cum = horzcat(cnn_lstm_fit_cum, h_cmp_cnn_lstm_fit_1min);
    cnn_lstm_amp_cum = horzcat(cnn_lstm_amp_cum, h_cmp_cnn_lstm_amp_1min);
    bnn_lstm_fit_cum = horzcat(bnn_lstm_fit_cum, h_cmp_bnn_lstm_fit_1min);
    bnn_lstm_amp_cum = horzcat(bnn_lstm_amp_cum, h_cmp_bnn_lstm_amp_1min);
    ...
    err_cnn_lstm_fit = [mean(abs(h_cmp_truth_1min - h_cmp_cnn_lstm_fit_1min), 'omitnan') mean(abs(t_cmp_truth_1min - t_cmp_cnn_lstm_fit_1min), 'omitnan')];
    err_cnn_lstm_amp = [mean(abs(h_cmp_truth_1min - h_cmp_cnn_lstm_amp_1min), 'omitnan') mean(abs(t_cmp_truth_1min - t_cmp_cnn_lstm_amp_1min), 'omitnan')];
    err_bnn_lstm_fit = [mean(abs(h_cmp_truth_1min - h_cmp_bnn_lstm_fit_1min), 'omitnan') mean(abs(t_cmp_truth_1min - t_cmp_bnn_lstm_fit_1min), 'omitnan')];
    err_bnn_lstm_amp = [mean(abs(h_cmp_truth_1min - h_cmp_bnn_lstm_amp_1min), 'omitnan') mean(abs(t_cmp_truth_1min - t_cmp_bnn_lstm_amp_1min), 'omitnan')];
    ...
    LSTM(Num_Windows,1) = err_lstm(1); 
    CNN_LSTM_Fit(Num_Windows,1) = err_cnn_lstm_fit(1);
    CNN_LSTM_Slow(Num_Windows,1) = err_cnn_lstm_amp(1);
    BNN_LSTM_Fit(Num_Windows,1) = err_bnn_lstm_fit(1);
    BNN_LSTM_Slow(Num_Windows,1) = err_bnn_lstm_amp(1);
    SSA(Num_Windows,1) = err_ssa(1); 
    Fit(Num_Windows,1) = err_fit(1); 
    Baseline(Num_Windows,1) = err_baseline(1);
    
    t_LSTM(Num_Windows,1) = err_lstm(2); 
    t_CNN_LSTM_Fit(Num_Windows,1) = err_cnn_lstm_fit(2); 
    t_CNN_LSTM_Slow(Num_Windows,1) = err_cnn_lstm_amp(2);
    t_BNN_LSTM_Fit(Num_Windows,1) = err_bnn_lstm_fit(2); 
    t_BNN_LSTM_Slow(Num_Windows,1) = err_bnn_lstm_amp(2);
    t_SSA(Num_Windows,1) = err_ssa(2); 
    t_Fit(Num_Windows,1) = err_fit(2); 
    t_Baseline(Num_Windows,1) = err_baseline(2);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    err_cnn_lstm_fit = [sqrt(mean((h_cmp_truth_1min-h_cmp_cnn_lstm_fit_1min).*(h_cmp_truth_1min-h_cmp_cnn_lstm_fit_1min), 'omitnan')) immse(t_cmp_truth_1min, t_cmp_cnn_lstm_fit_1min)];
    err_cnn_lstm_amp = [sqrt(mean((h_cmp_truth_1min-h_cmp_cnn_lstm_amp_1min).*(h_cmp_truth_1min-h_cmp_cnn_lstm_amp_1min), 'omitnan')) immse(t_cmp_truth_1min, t_cmp_cnn_lstm_amp_1min)];
    err_bnn_lstm_fit = [sqrt(mean((h_cmp_truth_1min-h_cmp_bnn_lstm_fit_1min).*(h_cmp_truth_1min-h_cmp_bnn_lstm_fit_1min), 'omitnan')) immse(t_cmp_truth_1min, t_cmp_cnn_lstm_fit_1min)];
    err_bnn_lstm_amp = [sqrt(mean((h_cmp_truth_1min-h_cmp_bnn_lstm_amp_1min).*(h_cmp_truth_1min-h_cmp_bnn_lstm_amp_1min), 'omitnan')) immse(t_cmp_truth_1min, t_cmp_cnn_lstm_amp_1min)];
    ...
    LSTM(Num_Windows,2) = err_lstm(1); 
    CNN_LSTM_Fit(Num_Windows,2) = err_cnn_lstm_fit(1);
    CNN_LSTM_Slow(Num_Windows,2) = err_cnn_lstm_amp(1); 
    BNN_LSTM_Fit(Num_Windows,2) = err_bnn_lstm_fit(1);
    BNN_LSTM_Slow(Num_Windows,2) = err_bnn_lstm_amp(1); 
    SSA(Num_Windows,2) = err_ssa(1); 
    Fit(Num_Windows,2) = err_fit(1); 
    Baseline(Num_Windows,2) = err_baseline(1);
    
    t_LSTM(Num_Windows,2) = err_lstm(2); 
    t_CNN_LSTM_Fit(Num_Windows,2) = err_cnn_lstm_fit(2);
    t_CNN_LSTM_Slow(Num_Windows,2) = err_cnn_lstm_amp(2);
    t_BNN_LSTM_Fit(Num_Windows,2) = err_bnn_lstm_fit(2);
    t_BNN_LSTM_Slow(Num_Windows,2) = err_bnn_lstm_amp(2);
    t_SSA(Num_Windows,2) = err_ssa(2); 
    t_Fit(Num_Windows,2) = err_fit(2); 
    t_Baseline(Num_Windows,2) = err_baseline(2);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    %%%%%%%%% ERRORS IN R2 %%%%%%%%%%
    err_lstm = corr(h_cmp_truth_1min', h_cmp_lstm_1min');
    err_cnn_lstm_fit = corr(h_cmp_truth_1min', h_cmp_cnn_lstm_fit_1min');
    err_cnn_lstm_amp = corr(h_cmp_truth_1min', h_cmp_cnn_lstm_amp_1min');
    err_bnn_lstm_fit = corr(h_cmp_truth_1min', h_cmp_bnn_lstm_fit_1min');
    err_bnn_lstm_amp = corr(h_cmp_truth_1min', h_cmp_bnn_lstm_amp_1min');
    ...
    LSTM(Num_Windows,3) = err_lstm; 
    CNN_LSTM_Fit(Num_Windows,3) = err_cnn_lstm_fit;
    CNN_LSTM_Slow(Num_Windows,3) = err_cnn_lstm_amp;
    BNN_LSTM_Fit(Num_Windows,3) = err_bnn_lstm_fit;
    BNN_LSTM_Slow(Num_Windows,3) = err_bnn_lstm_amp;
    SSA(Num_Windows,3) = err_ssa; 
    Fit(Num_Windows,3) = err_fit; 
    Baseline(Num_Windows,3) = err_baseline;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [max_wave_cnn_lstm_fit,~]=max(abs(cnn_lstm_fit(1:idx_1min)));
    [max_wave_cnn_lstm_amp,~]=max(abs(cnn_lstm_amps(1:idx_1min)));
    %[max_wave_bnn_lstm_fit,~]=max(abs(bnn_lstm_fit(1:idx_1min)));
    %[max_wave_bnn_lstm_amp,~]=max(abs(bnn_lstm_amps(1:idx_1min)));
    
    Max_LSTM(Num_Windows) = abs(max_wave_truth - max_wave_lstm);
    Max_CNN_LSTM_Fit(Num_Windows) = abs(max_wave_truth - max_wave_cnn_lstm_fit);
    Max_CNN_LSTM_Slow(Num_Windows) = abs(max_wave_truth - max_wave_cnn_lstm_amp);
    %Max_BNN_LSTM_Fit(i) = abs(max_wave_truth - max_wave_bnn_lstm_fit);
    %Max_BNN_LSTM_Slow(i) = abs(max_wave_truth - max_wave_bnn_lstm_amp);
    Max_SSA(Num_Windows) = abs(max_wave_truth - max_wave_ssa);
    Max_Fit(Num_Windows) = abs(max_wave_truth - max_wave_fit);
    Max_Baseline(Num_Windows) = abs(max_wave_truth - max_wave_baseline);
    
    %%%%%%%%%%%%%%%%% DATAFILES %%%%%%%%%%%%%%%%%%%%
    writematrix(LSTM,'LSTM_errors.txt','Delimiter','tab');
    writematrix(CNN_LSTM_Fit,'CNN_LSTM_Fit_errors.txt','Delimiter','tab');
    writematrix(CNN_LSTM_Slow,'CNN_LSTM_Slow_errors.txt','Delimiter','tab');
    writematrix(BNN_LSTM_Fit,'BNN_LSTM_Fit_errors.txt','Delimiter','tab');
    writematrix(BNN_LSTM_Slow,'BNN_LSTM_Slow_errors.txt','Delimiter','tab');
    ...
    writematrix(t_CNN_LSTM_Fit,'t_CNN_LSTM_Fit_errors.txt','Delimiter','tab');
    writematrix(t_CNN_LSTM_Slow,'t_CNN_LSTM_Slow_errors.txt','Delimiter','tab');
    writematrix(t_BNN_LSTM_Fit,'t_BNN_LSTM_Fit_errors.txt','Delimiter','tab');
    writematrix(t_BNN_LSTM_Slow,'t_BNN_LSTM_Slow_errors.txt','Delimiter','tab');
    ...
    writematrix(Max_CNN_LSTM_Fit,'CNN_LSTM_Fit_max_errors.txt','Delimiter','tab');
    writematrix(Max_CNN_LSTM_Slow,'CNN_LSTM_Slow_max_errors.txt','Delimiter','tab');
    %writematrix(Max_BNN_LSTM_Fit,'BNN_LSTM_Fit_max_errors.txt','Delimiter','tab');
    %writematrix(Max_BNN_LSTM_Slow,'BNN_LSTM_Slow_max_errors.txt','Delimiter','tab');
    ...
    writematrix(cnn_lstm_fit_cum,'CNN_LSTM_Fit_cumulative.txt');
    writematrix(cnn_lstm_amp_cum,'CNN_LSTM_Slow_cumulative.txt');
    writematrix(bnn_lstm_fit_cum,'BNN_LSTM_Fit_cumulative.txt');
    writematrix(bnn_lstm_amp_cum,'BNN_LSTM_Slow_cumulative.txt');
