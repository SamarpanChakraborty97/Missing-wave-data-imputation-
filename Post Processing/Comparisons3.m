...
%     hz_slowflow_full_baseline=SF2fulltime(t_miss_plot,t_miss_plot,baseline,freqs,N_fmodes);
    ...
    %%%%%% WAVES FOR FREQUENCY COMPARISONS %%%%%%
    L = length(true_time);
    if (L < min_peaks)
        lstm_min_time_series = hz_slowflow_full_lstm;
        cnn_lstm_min_time_series = cnn_lstm;
        ssa_min_time_series = ssa;
        fit_min_time_series = hz_slowflow_full_fit;
        true_min_time_series = h_miss_plot;
        
        min_peaks = L;
    end
        
    if (L > max_peaks)
        lstm_max_time_series = hz_slowflow_full_lstm;
        cnn_lstm_max_time_series = cnn_lstm;
        ssa_max_time_series = ssa;
        fit_max_time_series = hz_slowflow_full_fit;
        true_max_time_series = h_miss_plot;
...
writematrix(lstm_min_time_series,'LSTM_min.txt','Delimiter','tab');
writematrix(cnn_lstm_min_time_series,'CNN_LSTM_min.txt','Delimiter','tab');
writematrix(ssa_min_time_series,'SSA_min.txt','Delimiter','tab');
writematrix(fit_min_time_series,'Fit_min.txt','Delimiter','tab');
writematrix(true_min_time_series,'Truth_min.txt','Delimiter','tab');

writematrix(lstm_max_time_series,'LSTM_max.txt','Delimiter','tab');
writematrix(cnn_lstm_max_time_series,'CNN_LSTM_max.txt','Delimiter','tab');
writematrix(ssa_max_time_series,'SSA_max.txt','Delimiter','tab');
writematrix(fit_max_time_series,'Fit_max.txt','Delimiter','tab');
writematrix(true_max_time_series,'Truth_max.txt','Delimiter','tab');
