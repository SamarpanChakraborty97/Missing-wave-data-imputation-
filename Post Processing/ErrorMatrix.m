...
Num_Windows = 1;
initial_window_start = randi([0, 100]);
...
for i=1:Num_Windows
    ...
    [h_cmp_baseline, t_cmp_baseline] = my_wave_height_compare(true_height,true_time,baseline_wave_height,baseline_time);
    ...
    subplot(4,1,1);
    plot(t_cmp_truth_1min, h_cmp_truth_1min, 'k-.','Linewidth',1.0);
    hold on;
    plot(t_cmp_lstm_1min, h_cmp_lstm_1min, 'b-.','Linewidth',1.0);
    legend(['Truth peaks','LSTM prediction peaks'],'FontName','Times','FontSize',10,'interpreter','latex');
    
    subplot(4,1,2);
    plot(t_cmp_truth_1min, h_cmp_truth_1min, 'k-.','Linewidth',1.0);
    hold on;
    plot(t_cmp_cnn_lstm_1min, h_cmp_cnn_lstm_1min, 'b-.','Linewidth',1.0);
    legend(['Truth peaks','CNN+LSTM prediction peaks'],'FontName','Times','FontSize',10,'interpreter','latex');
    
    subplot(4,1,3);
    plot(t_cmp_truth_1min, h_cmp_truth_1min, 'k-.','Linewidth',1.0);
    hold on;
    plot(t_cmp_ssa_1min, h_cmp_ssa_1min, 'b-.','Linewidth',1.0);
    legend(['Truth peaks','SSA prediction peaks'],'FontName','Times','FontSize',10,'interpreter','latex');
    
    subplot(4,1,4);
    plot(t_cmp_truth_1min, h_cmp_truth_1min, 'k-.','Linewidth',1.0);
    hold on;
    plot(t_cmp_fit_1min, h_cmp_fit_1min, 'b-.','Linewidth',1.0);
    legend(['Truth peaks','Fit peaks'],'FontName','Times','FontSize',10,'interpreter','latex');
    
    g.PaperUnits = 'inches';
    g.PaperPosition = [0 0 8 8];
    name2 = sprintf('Sample comparisons plots for %i minute for different models over %i windows.tiff',mins,Num_Windows);
...
%     
%     %%%%%%%%% ERRORS IN MAE %%%%%%%%%%
%     err_lstm = [mean(abs(h_cmp_truth_1min - h_cmp_lstm_1min)) mean(abs(t_cmp_truth_1min - t_cmp_lstm_1min))];
%     err_cnn_lstm = [mean(abs(h_cmp_truth_1min - h_cmp_cnn_lstm_1min)) mean(abs(t_cmp_truth_1min - t_cmp_cnn_lstm_1min))];
%     err_ssa = [mean(abs(h_cmp_truth_1min - h_cmp_ssa_1min)) mean(abs(t_cmp_truth_1min - t_cmp_ssa_1min))];
%     err_fit = [mean(abs(h_cmp_truth_1min - h_cmp_fit_1min)) mean(abs(t_cmp_truth_1min - t_cmp_fit_1min))];
%     err_baseline = [mean(abs(h_cmp_truth_1min - h_cmp_baseline_1min)) mean(abs(t_cmp_truth_1min - t_cmp_baseline_1min))];
%     
%     LSTM(i,1) = err_lstm(1); 
...
%     
%     %%%%%%%%% ERRORS IN MSE %%%%%%%%%%
%     err_lstm = [immse(h_cmp_truth_1min, h_cmp_lstm_1min) immse(t_cmp_truth_1min, t_cmp_lstm_1min)];
%     err_cnn_lstm = [immse(h_cmp_truth_1min, h_cmp_cnn_lstm_1min) immse(t_cmp_truth_1min, t_cmp_cnn_lstm_1min)];
%     err_ssa = [immse(h_cmp_truth_1min, h_cmp_ssa_1min) immse(t_cmp_truth_1min, t_cmp_ssa_1min)];
%     err_fit = [immse(h_cmp_truth_1min, h_cmp_fit_1min) immse(t_cmp_truth_1min, t_cmp_fit_1min)];
%     err_baseline = [immse(h_cmp_truth_1min, h_cmp_baseline_1min) immse(t_cmp_truth_1min, t_cmp_baseline_1min)];
%     
%     LSTM(i,2) = err_lstm(1); 
...
%     [max_wave_baseline,~]=max(abs(hz_slowflow_full_baseline(1:idx_1min)));
